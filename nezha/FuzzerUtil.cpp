...
// ssdeep-lib for fuzzy hash bucketing
#include <fuzzy.h>
...
std::string Hash(const Unit &U) {
  return HashSha1(U.data(), U.size());
}

std::string HashFuzzy(const uint8_t *Data, size_t Size) {
  uint8_t Hash[FUZZY_MAX_RESULT];
  int r = fuzzy_hash_buf(Data, Size, (char *)Hash);
  assert(r == 0 && "Fuzzy hash computation failed");
  
  std::stringstream SS;
  for (int i = 0; i < FUZZY_MAX_RESULT; i++)
    SS << Hash[i];
  return SS.str();
}

int HashFuzzy_cmp(std::string fuzzysig1, std::string fuzzysig2) {
  int score = fuzzy_compare(fuzzysig1.c_str(), fuzzysig2.c_str());
  assert(score >= 0 && "Fuzzy hash comparison failed");
  return score;
}
