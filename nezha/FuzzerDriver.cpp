...
int FuzzerDriver(int *argc, char ***argv, UserCallback Callback) {
  ...
  Options.ForceDefault = Flags.force_default;
  Options.DiffFhashMin = Flags.fhash_min;
  Options.LogUnique = Flags.log_unique;
  Options.GlobalCoverage = Flags.diff_union;
  Options.PDCoarse = Flags.diff_pdcoarse;
  Options.PDFine = Flags.diff_pdfine;
  Options.OD = Flags.diff_od;
  ...
  if (Flags.verbosity) {
    Printf("INFO: Seed: %u\n", Seed);
    if (Options.OD || Options.PDCoarse ||
        Options.PDFine || Options.GlobalCoverage) {
      std::string mode = "";
      if (Options.GlobalCoverage) mode = "Global Coverage";
      if (Options.PDCoarse) mode = "Path Diversity (Coarse)";
      if (Options.PDFine) mode = "Path Diversity (Fine)";
      if (Options.OD) mode = "Output Diversity";
      Printf("INFO: DiffMode: | %s\n", mode.c_str());
    }
  }
