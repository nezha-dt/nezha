...
std::string HashSha1(const uint8_t *Data, size_t Size);
std::string HashFuzzy(const uint8_t *Data, size_t Size);
int HashFuzzy_cmp(std::string fuzzysig1, std::string fuzzysig2);
...
// Ref: http://stackoverflow.com/questions/29855908/c-unordered-set-of-vectors
...
using SetOfVector = std::unordered_set<std::vector<int>, VectorIntHash>;
...
public:
  ...
  struct Coverage {
    ...
    // For tracking global coverage.
    std::unordered_set<size_t> PCSet;

    // For tracking global bitcount.
    uint32_t LastCounterDelta;
  };

  // Aggregates all available difference-based measurements.
  struct Diff {
    Diff() { Reset(); }
    ...
    std::vector<std::pair <std::string, std::pair <std::vector<std::string>,
      std::vector<int> > > > DiffHashes;
    std::unordered_set<std::string> SetCovPaths;
    SetOfVector SetOutputs;
    SetOfVector SetCovDiffs;
    SetOfVector SetRawEcDiffs;
  };
...
private:
  ...
  // Update difference-related statistics and log differences
  bool UpdateDiffAndLog(const uint8_t *Data, size_t Size);

  // Check for external functions required for difference-based fuzzing.
  void CheckDiffBasedFuncs();
